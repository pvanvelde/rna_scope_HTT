\caption{\textbf{Filtering spots based on size and intensity criteria after empirical PSF calibration.}
...
\textbf{(A)} Raw integrated photon counts for each of 15 slides (faint colored lines show kernel density estimates for individual slides, 176,808 total spots, \textbf{green channel}).
...
Purple dotted vertical lines mark the bead-derived PSF from fluorescent microsphere calibration ($\sigma_x = 263$~nm, $\sigma_y = 263$~nm, $\sigma_z = 666$~nm), representing the diffraction-limited optical reference.
Colored dashed vertical lines indicate empirically-determined breakpoints ($\sigma_x = 290.8$~nm, $\sigma_y = 291.1$~nm, $\sigma_z = 698.6$~nm), which exceed bead PSF by 11\%, 11\%, and 5\% respectively---consistent with the finite physical extent of RNAscope probe clusters ($\sim$20 probe pairs, each $\sim$2--3~nm) bound to target mRNA in tissue.
...
(2) Empirically-determined breakpoints exceed bead PSF by 11--5\%, consistent with the physical extent of RNAscope probe clusters bound to target mRNA.
...
(5) Strict filtering criteria (size $< \mathrm{bp}$, $p_{\mathrm{FA}} < 5e-02$, intensity $>$ threshold) yield 176,808 high-confidence single molecules across 15 slides.
%
\textbf{Methods:}
Analysis performed on 176,808 single spots and 2,435,017 clusters across 15 slides from Q111 transgenic mouse tissue, using RNAscope \emph{in situ} hybridization with fluorescent detection.
