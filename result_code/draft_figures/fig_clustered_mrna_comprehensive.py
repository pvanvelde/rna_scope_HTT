"""
Comprehensive Clustered mRNA Expression Figure
...
# Output directory
OUTPUT_DIR = Path(__file__).parent / "output" / "clustered_mrna_comprehensive"
...
def create_comprehensive_figure(df_fov):
    ...
    # Filter for clustered mRNA only
    df_fov = df_fov.copy()
    ...
    for col_idx, region in enumerate(['Cortex', 'Striatum']):
        ...
        for age in sorted(df_region['Age'].unique()):
            for model in ['Q111', 'Wildtype']:
                for channel in ['HTT1a', 'fl-HTT']:
                    ...
                    for val in subset['Clustered_mRNA_per_Cell'].values:
                        if not np.isnan(val):
                            plot_data.append({
                                'Age': age,
                                'Model': model,
        ...
        ax.set_ylabel('Clustered mRNA/nucleus', fontsize=12, fontweight='bold')
        ax.set_title(f'{chr(65 + col_idx)}) {region}: Clustered mRNA by Age',
    ...
    for col_idx, region in enumerate(['Cortex', 'Striatum']):
        ...
        ax.set_ylabel('Clustered mRNA/nucleus', fontsize=12, fontweight='bold')
        ax.set_title(f'{chr(67 + col_idx)}) {region}: Clustered mRNA by Atlas Position',
                    fontsize=13, fontweight='bold', pad=15)
        ax.grid(axis='y', alpha=0.3, linestyle='--', linewidth=0.5)
    ...
    for col_idx, region in enumerate(['Cortex', 'Striatum']):
        ...
        for mouse_id in sorted(q111_mouse_ids, key=lambda x: (extract_mouse_info(x), x)):
            ...
            for channel in ['HTT1a', 'fl-HTT']:
                ...
                if len(channel_subset) > 0:
                    mean_val = channel_subset['Clustered_mRNA_per_Cell'].mean()
                    sem_val = channel_subset['Clustered_mRNA_per_Cell'].sem()
        ...
        ax.set_ylabel('Clustered mRNA/nucleus', fontsize=14, fontweight='bold')
    ...
    caption_lines.append("FIGURE: Comprehensive Clustered mRNA Expression Analysis")
    caption_lines.append("=" * 80)
    ...
    caption_lines.append("-" * 80)
    caption_lines.append("This figure presents a comprehensive analysis of clustered mRNA expression in Q111")
    ...
    caption_lines.append("striatal brain regions. Clustered mRNA represents aggregated transcripts with")
    caption_lines.append("high local concentration, quantified as: (I_cluster_total / I_single_peak) / N_nuclei.")
    caption_lines.append("")
    ...
    caption_lines.append("Violin plots showing the distribution of clustered mRNA expression across ages.")
    ...
    caption_lines.append("Box plots showing clustered mRNA expression across anterior-posterior brain atlas")
    ...
    caption_lines.append("Bar plots showing mean±SEM clustered mRNA expression for each individual slide,")
    ...
    caption_lines.append("  - Clustered mRNA/nucleus = (I_cluster_total / I_single_peak) / N_nuclei")
    caption_lines.append("")
    ...
    for fmt in ['png', 'svg', 'pdf']:
        output_path = OUTPUT_DIR / f"fig_clustered_mrna_comprehensive.{fmt}"
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("COMPREHENSIVE CLUSTERED mRNA EXPRESSION FIGURE")
    print("="*70)
