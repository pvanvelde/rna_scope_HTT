...
if __name__ == "__main__":
    ...
    # Breakpoints and PSF values for reference lines (from CHANNEL_PARAMS, using green channel)
    green_sigma = CHANNEL_PARAMS['green']['sigma']
    green_break = CHANNEL_PARAMS['green']['break_sigma']

    breakpoints = {
        'sigma_x': green_break[0] * pixelsize,  # nm
        'sigma_y': green_break[1] * pixelsize,  # nm
        'sigma_z': green_break[2] * slice_depth  # nm
    }

    psf_values = {
        'sigma_x': green_sigma[0] * pixelsize,  # nm
        'sigma_y': green_sigma[1] * pixelsize,  # nm
        'sigma_z': green_sigma[2] * slice_depth  # nm
    }

    print(f"Using PSF values from CHANNEL_PARAMS (green):")
    ...
    # We'll focus on green channel, ExperimentalQ111 probe set
    df_to_plot = df_single[
        (df_single['channel'] == 'green') &
        (df_single['probe_set'] == 'ExperimentalQ111 - 488mHT - 548mHTa - 647Darp')
    ]

    print(f"\nFiltered to green channel, ExperimentalQ111: {df_to_plot.shape[0]} spots")
    ...
    output_filename = "fig_filtering_v2"
    ...
    caption = f"""\\caption{{\\textbf{{Filtering spots based on size and intensity criteria after empirical PSF calibration.}}
\\textbf{{Note:}} All panels show data from the \\textbf{{green channel}} (488~nm excitation) only, corresponding to the ExperimentalQ111 probe set in Q111 transgenic mouse tissue.
Analysis of orange channel (548~nm) data yields similar results (not shown).
\\textbf{{(A--B)}} Per-slide intensity normalization demonstrates successful removal of technical variation.
\\textbf{{(A)}} Raw integrated photon counts for each of {n_slides} slides (faint colored lines show kernel density estimates for individual slides, {n_spots_total:,} total spots, \\textbf{{green channel}}).
...
\\textbf{{Channel analyzed:}} Green channel (488~nm excitation wavelength, detecting fl-HTT mRNA via fluorophore-conjugated probes).
